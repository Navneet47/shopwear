        ...
        <div className="bg-white">
            <div className="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8">
                ...
                <div className="grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-8">
                    {Object.keys(products).length === 0 && <p>Sorry! all Hoodies are currently out of stock. New Stock Coming Soon. Stay tuned!</p> }
                    {Object.keys(products).map((item) => {
                        return <Link passHref={true} key={products[item]._id} href={`/product/${products[item].slug}`} className="group shadow-2xl">
                            ...
                            <h3 className="mt-4 text-sm text-gray-700 pl-2">Hoodies</h3>
