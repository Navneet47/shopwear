...
function Hoodies({ products, allCount }) {
    ...
    const fetchData = async () => {
        const res = await fetch(`${process.env.NEXT_PUBLIC_HOST}/api/getproducts?count=${count + 9}`, {
            ...
            body: JSON.stringify({ category: 'hoodie' }),
        }).then((t) => t.json());
        setCount(count + 9);
        setProductList(res);
    }
    ...
    return (
        <div>
            <Head>
                <title>Buy Hoodies at best price in India - Shopnation.com</title>
            ...
            <header>
                <div className="flex flex-col mx-14 lg:mx-32">
                    ...
                    <p className="text-sm font-medium dark:text-gray-400 mb-3">
                        Stay warm and stylish with the wide selection of hoodies available at Shopnation.com. All of our hoodies are made with high-quality materials and are designed to be comfortable and durable. Shop now and find the perfect hoodies for you!
                    </p>
                    {products.length === 0 && <p className="font-bold text-xl mt-20">Sorry all the Hoodies are currently out of stock. New stock coming soon. Stay Tuned!</p>}
                </div>
            </header>
    ...
    for (let item of products) {
        if (item.title in hoodie) {
            ...
            if (item.availableQty > 0) {
                ...
                hoodie[item.title].color = []
                hoodie[item.title].size = []
            }
        }
    }
    Object.keys(hoodie).map((item) => {
        result.push(hoodie[item])
    })
