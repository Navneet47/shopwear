...
function Tshirts({ products, allCount }) {
  ...
  const fetchData = async () => {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_HOST}/api/getproducts?count=${count + 9}`,
      {
        method: "POST",
        header: {
  ...
  return (
    <div>
      <Head>
        <title>Buy Tshirts at best price in India - Shopnation.in</title>
        <meta
          name="viewport"
      ...
      <header>
        <div className="flex flex-col mx-14 lg:mx-32">
          ...
          <p className="text-sm font-medium dark:text-gray-400 mb-3">
            Stay warm and stylish with the wide selection of Tshirts available
            at Shopnation.com. All of our Tshirts are made with high-quality
            materials and are designed to be comfortable and durable. Shop now
            and find the perfect Tshirts for you!
          </p>
          {products.length === 0 && (
